...
explanation : "grow" the specifed board by taking every potentially possible moved board as the root of substree, ie. by using moves, and continuously using "moves" to the childs in the list
              until there is no possible move, ie, the board is full, take the full board as Leaf (with empty substree)
argument    : just board
output: the whole possible board tree generated from the specified board
...
prune    
goal : the outcame of the growTree function is too big, we want to be prune it down to specified depth 
explanation: take the depth as parameter, decrease it each time when we recursively call it for the childs until the depth is 0 which means no childs is needed.
argument : a int type number which is used to specify the depth; the board
output: the tree with boards in nodes
...
minimax
goal : mark each node(board) with a player (Player, Board) who will win if the player take reasonable move every time. if we cannot tell who will win, make it as 'B'
explanation:Due to natually O < B < X, by the theory of minimax, when its X turn, take the maximum of next move; when its O's turn, take the minimum of next move. 
            use (Player,Board) instead of (Board,Player) actually make a difference. 
            when we deside which player should be marked for the board by checking the outcome of one step forward, we actually only need to compare the palyer.
            instead of using (Board,Player) in which we have to generate a list of Player and choose the reasonable player from it, using (Player, Board) where you compare the tree directly with higher effciency. though there are 
            
            
            there are tow situation when no childs (empty list) will occur: i.the board is full ii.reach to the specified depth. In either case, if we cannot tell who win, we mark it as 'B'

> minimax :: Tree Board -> Tree (Player, Board)
> minimax (Node b cs)
>            | hasWon X b = Node (X,b) []
>            | hasWon O b = Node (O,b) []
>            | cs == [] = Node (B,b) [] --on child but cannot judge who wins, mark it as "B"
>            | turn b == X = Node (px,b) childs
>            | otherwise = Node (po,b) childs --turn b == O
>                     where Node (px,_) _ = maximum childs
>                           Node (po,_) _ = minimum childs 
...
> timethe a = do start <- getCurrentTime
>                let b = a
>                stop <- getCurrentTime
...
> test1 = [[B,B,B,B,B,B,B],[B,B,B,B,B,B,B],[B,B,B,B,B,B,B],[B,B,B,B,B,B,B],[B,B,B,B,B,X,B],[B,B,B,B,O,B,B]];
> test2 = [[B,B,B,B,B,B,B],[B,B,B,B,B,B,B],[B,B,B,B,B,B,B],[B,B,B,B,B,B,B],[B,B,B,B,B,B,B],[X,B,B,B,O,B,B]];
