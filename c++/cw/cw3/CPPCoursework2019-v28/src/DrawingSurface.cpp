...
*/
void DrawingSurface::copyRectangleFrom(DrawingSurface* pFrom, int iRealX, int iRealY, int iWidth, int iHeight)
{
	if (iRealX + iWidth < 0)
	...
	if (iRealX < 0) { iRealX = 0; iWidth += iRealX; /*Note that iRealX is negative*/ }
	if (iRealY < 0) { iRealY = 0; iHeight += iRealY; /*Note that iRealX is negative*/ }
	...
	int iStart = iRealY * getIntsPerWindowRow() + iRealX;
	int iIncrement = getIntsPerWindowRow() - iWidth;

	unsigned int * puiSource = ((unsigned int *)pFrom->m_pSDLSurface->pixels) + iStart;
	unsigned int * puiDest = ((unsigned int *)m_pSDLSurface->pixels) + iStart;
	...
	unsigned int uiColour)
{
	if (iPoints == 1)
	{
		setPixel(pXArray[0], pYArray[0], uiColour);
		return;
	}
