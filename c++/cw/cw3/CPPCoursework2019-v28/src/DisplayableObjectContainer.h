...
public:
	...
	int getContentCount() { return m_vecDisplayableObjects.size();  }

	// Get a count of how many things are in container
	DisplayableObject* getContentItem( int iIndex ) { return m_vecDisplayableObjects[iIndex]; }
	...
	*/
	...
	bool moveToLast(DisplayableObject* pObject)
	{
		...
		{ // Found the pointer
			...
			pObject->virtRedrawRectangle(); // Ensure that object is redrawn in its new position, in case we are doing partial updates now
			return true;
		}
	...
	*/
	...
	int getNonNullObjectContentCount()
	{
		int iReturn = 0;
	...
	*/
	...
	void notifyAllObjects(int iSignalNumber, int x = 0, int y = 0)
	{
	...
	int notifyAllObjectsGetCountNonZero(int iSignalNumber, int x = 0, int y = 0)
	{
		int iReturn = 0;
	...
	int notifyAllObjectsGetSum(int iSignalNumber, int x = 0, int y = 0)
	{
		int iReturn = 0;
	...
	int notifyAllObjectsGetMax(int iSignalNumber, int x = 0, int y = 0)
	{
		int iReturn = INT_MIN;
	...
	int notifyAllObjectsGetMin(int iSignalNumber, int x = 0, int y = 0)
	{
		int iReturn = INT_MAX;
		std::for_each(m_vecDisplayableObjects.begin(), m_vecDisplayableObjects.end(), [=, &iReturn](DisplayableObject* pdo) { if (pdo == nullptr) return; int ival = pdo->virtNotify(iSignalNumber, x, y); if (ival < iReturn) iReturn = ival; });
		return iReturn;
	...
	/* Send a specified notification value to all displayable objects and return the smallest of the returned values. */
	void setAllObjectsVisible(bool bVisible)
	{
	...
	DisplayableObject* getBottomObjectForNotifyValue(int iSignalNumber, int iValue, int x, int y)
	{
		for (int i = 0; i < m_vecDisplayableObjects.size(); i++)
	...
	DisplayableObject* getTopObjectForNotifyValue(int iSignalNumber, int iValue, int x, int y)
	{
	...
	void applyToAll(std::function<void(DisplayableObject* ob)> f)
	{
	...
	// NOTE: bookmark is cleared it items are removed/added to array!
	//
	...
	*/
	...
	bool bookmarkSet(int iIndex) { m_iCurrentItemIndex = iIndex; }
