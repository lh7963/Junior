...
// See FilterPoints.h for what these methods do
...
public:
	...
	*/
	...
	virtual int convertXPixelVirtualToReal(int xVirtual) override
	{
		if (xStretch > 1)
		{
			if (pNextFilter)
				return pNextFilter->convertXPixelVirtualToReal(xVirtual * xStretch);
			else
		...
		{
			if (pNextFilter)
				return pNextFilter->convertXPixelVirtualToReal(xVirtual / xCompress);
			else
	...
	// Calculate real from virtual position - apply the stretch then any following filter
	virtual int convertYPixelVirtualToReal(int yVirtual) override
	{
		if (yStretch > 1)
		{
			if (pNextFilter)
				return pNextFilter->convertYPixelVirtualToReal(yVirtual * yStretch);
			else
		...
		{
			if (pNextFilter)
				return pNextFilter->convertYPixelVirtualToReal(yVirtual / xCompress);
			else
	...
	virtual int convertXClickRealToVirtual(int xReal) override
	{
		// Apply other filter first if it exists (since we are reversing the order we used for virtual to real)
		if (pNextFilter)
			xReal = pNextFilter->convertXClickRealToVirtual(xReal);
	...
	// These convert from a clicked location to a virtual pixel position in a surface
	virtual int convertYClickRealToVirtual(int yReal) override
	{
		// Apply other filter first if it exists (since we are reversing the order we used for virtual to real)
		if (pNextFilter)
			yReal = pNextFilter->convertYClickRealToVirtual(yReal);
		// Other filter has now been applied so we can try to reverse the effect of THIS filter...
		if (yStretch > 1)
...
public:
	...
	virtual int convertXPixelVirtualToReal(int xVirtual) override
	{
		...
		if (pNextFilter)
			return pNextFilter->convertXPixelVirtualToReal(xVirtual + xModifier);
	...
	virtual int convertYPixelVirtualToReal(int yVirtual) override
	{
		// Apply other filter first if it exists (since we are reversing drawing order)
		if (pNextFilter)
			return pNextFilter->convertYPixelVirtualToReal(yVirtual + yModifier);
	...
	virtual int convertXClickRealToVirtual(int x) override
	{
		// Apply other filter first if it exists (since we are reversing drawing order)
		if (pNextFilter)
			x = pNextFilter->convertXClickRealToVirtual(x);
		return x - xModifier;
	}

	// These convert from a clicked location to a virtual pixel position in a surface
	virtual int convertYClickRealToVirtual(int y) override
	{
		// Apply other filter first if it exists (since we are reversing drawing order)
		if (pNextFilter)
			y = pNextFilter->convertYClickRealToVirtual(y);
		return y - yModifier;
	}
